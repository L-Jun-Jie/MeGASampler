...
class SatisfiesMetric(Metric):
    ...
    def _add_sample_via_smtlib(self, sample: list[tuple[str, int]]):
        ...
        # 打印生成的SMT-LIB字符串，方便检查格式
        # print("Generated SMT-LIB String:", smtlib_str)

        # 加载到求解器
        self._solver.from_string(smtlib_str)
...
class WireCoverageStatistics(NodeStatistics):
    ...
    def result(self) -> fractions.Fraction:
        ...
        fraction = fractions.Fraction(count, total)
        percentage = float(fraction) * 100  # 将分数转换为百分数
        return f"{percentage:.2f}%"  # 将结果格式化为带两位小数的百分数字符串
